...
function friendlyError(raw: string): string {
  ...
  if (
    ...
    return "Erreur de configuration du service WhatsApp. Veuillez réessayer plus tard.";
  }
  return raw;
}
...
export function PaymentMethodModal({
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      setError(
        friendlyError(err instanceof Error ? err.message : "Erreur réseau. Veuillez réessayer."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirm = async () => {
    ...
    if (code.length < OTP_LENGTH) {
      setError(`Entrez le code à ${OTP_LENGTH} chiffres reçu par WhatsApp.`);
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/momo/verify-and-save`, {
        ...
        body: JSON.stringify({ phoneNumber: phoneInternational, provider, code }),
      });
      const data = (await res.json()) as { error?: string };
          ...
          {step === "form" ? (
            <>
              ...
              {userType === "PRO" && (
                <>
                  ...
                  <TextInput
                    style={[styles.textField, { backgroundColor: colors.input, borderColor: colors.border, color: colors.foreground }]}
                    placeholder="Ex: JEAN DUPONT"
                    placeholderTextColor={colors.mutedForeground}
                    autoCapitalize="characters"
                    value={holderName}
                    onChangeText={(v) => { setHolderName(v); setError(null); }}
                  />
                </>
              )}
              ...
              <Pressable
                ...
                disabled={loading}
                style={[styles.ctaButton, { backgroundColor: colors.primary, opacity: loading ? 0.7 : 1 }]}
              >
                {loading ? (
                  <ActivityIndicator color="#FFFFFF" />
                ) : (
                  <Text style={styles.ctaText}>VÉRIFIER LE NUMÉRO PAR WHATSAPP</Text>
                )}
              </Pressable>
            </>
          ) : (
            ...
            <>
              ...
              <View style={styles.resendRow}>
                {resendCooldown > 0 ? (
                  ...
                  <Pressable
                    ...
                    hitSlop={8}
                  >
                    <Text style={[styles.resendLink, { color: colors.primary, opacity: loading ? 0.5 : 1 }]}>
                      Renvoyer le code
                    </Text>
                  </Pressable>
                )}
              ...
              <View style={styles.otpActions}>
                <Pressable
                  onPress={() => { setStep("form"); setOtp(Array(OTP_LENGTH).fill("")); setError(null); }}
                  style={[styles.secondaryButton, { borderColor: colors.border }]}
                >
                  <Text style={[styles.secondaryButtonText, { color: colors.foreground }]}>RETOUR</Text>
                </Pressable>
                <Pressable
                  onPress={handleConfirm}
                  disabled={loading}
                  style={[styles.ctaButton, { flex: 1, backgroundColor: colors.primary, opacity: loading ? 0.7 : 1 }]}
                >
                  {loading ? (
                    <ActivityIndicator color="#FFFFFF" />
                  ) : (
