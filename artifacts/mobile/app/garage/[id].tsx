...
export default function GarageDetailScreen() {
  ...
  // Fire once per 24 h per garage, skipping own garage and preview mode.
  const viewRecorded = useRef(false);

  useEffect(() => {
    // Wait until we know whether this is the owner's own garage
    if (!garage.data) return;
    if (viewRecorded.current) return;
    if (isOwnGarage || isPreview) return;

    viewRecorded.current = true;

    (async () => {
      try {
        const storageKey = `garage_view_ts_${garageId}`;
        const lastTs = await AsyncStorage.getItem(storageKey);
        const now = Date.now();
        if (lastTs && now - Number(lastTs) < 24 * 60 * 60 * 1000) return;

        // Persist or create an anonymous viewer ID
        let viewerKey = await AsyncStorage.getItem("wapi_viewer_id");
        if (!viewerKey) {
          viewerKey =
            Math.random().toString(36).slice(2) +
            Math.random().toString(36).slice(2);
          await AsyncStorage.setItem("wapi_viewer_id", viewerKey);
        }
        ...
        await fetch(`https://${domain}/api/garages/${garageId}/views`, {
          method: "POST",
          headers: {
        ...
        // View tracking is best-effort — never block the UI
      }
    })();
  }, [garage.data, isOwnGarage, isPreview, garageId]);
