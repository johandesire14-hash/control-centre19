...
import { Pencil, ImageIcon, Wrench, Clock, Star, MessageCircle, Bell, User, Camera, Plus, Menu, X, ChevronRight, ChevronUp, ChevronDown, Eye } from "lucide-react-native";
...
/** Build an 8-week series (oldest → newest) filling zeros for missing weeks. */
function buildWeekSeries(
  weekly: { weekStart: string; count: number }[],
): { label: string; count: number; partial: boolean }[] {
  ...
  return Array.from({ length: 8 }, (_, idx) => {
    const weeksAgo = 7 - idx; // idx 0 = 7 weeks ago … idx 7 = current week
    const monday = new Date(thisMonday);
    monday.setDate(thisMonday.getDate() - weeksAgo * 7);

    const match = weekly.find((w) => {
      const ws = new Date(w.weekStart);
      ws.setHours(0, 0, 0, 0);
      return Math.abs(ws.getTime() - monday.getTime()) < 4 * 24 * 60 * 60 * 1000;
    });
...
/** Simple vertical bar chart — no external library required. */
function ViewsChart({
  G,
  weeks,
}: {
  G: GarageTheme;
  weeks: { label: string; count: number; partial: boolean }[];
}) {
  const MAX_H = 56;
  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "flex-end", gap: 3 }}>
      {weeks.map((w, i) => {
        const barH = Math.max((w.count / maxCount) * MAX_H, 3);
        return (
          <View key={i} style={{ flex: 1, alignItems: "center", gap: 3 }}>
            ...
            <View
              style={{
                ...
                height: barH,
                borderRadius: 4,
                backgroundColor: w.partial ? G.greenBg : G.green,
                borderWidth: w.partial ? 1 : 0,
                borderColor: G.green,
              }}
            />
            <Text
    ...
    (async () => {
      try {
        ...
        const res = await fetch(
          `https://${domain}/api/garages/mine/views/stats`,
          { headers: token ? { Authorization: `Bearer ${token}` } : {} },
        );
        if (res.ok) setViewStats(await res.json());
      } catch {
        // best-effort
      }
    })();
      ...
      >
        ...
        <View style={s.statsGrid}>
          <StatCard
            ...
            value={viewStats != null ? String(viewStats.thisMonth) : "—"}
            sub={viewStats != null ? "ce mois-ci" : undefined}
            delta={
              viewStats != null && viewStats.lastMonth > 0
                ? viewStats.thisMonth >= viewStats.lastMonth
                  ? `+${viewStats.thisMonth - viewStats.lastMonth} vs mois préc.`
                  : `−${viewStats.lastMonth - viewStats.thisMonth} vs mois préc.`
                : undefined
            }
            deltaPositive={
              viewStats != null ? viewStats.thisMonth >= viewStats.lastMonth : undefined
            }
          />
          <StatCard
            G={G}
        ...
        {viewStats != null && (
          ...
          >
            <ViewsChart G={G} weeks={buildWeekSeries(viewStats.weekly)} />
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 14 }}>
              <View style={{ gap: 2 }}>
                <Text style={{ fontFamily: "Inter_400Regular", fontSize: 11, color: G.muted }}>
                  Ce mois-ci
                </Text>
                <Text style={{ fontFamily: "Inter_700Bold", fontSize: 20, color: G.text }}>
                  {viewStats.thisMonth}
                </Text>
              </View>
              <View style={{ gap: 2, alignItems: "flex-end" }}>
                ...
                <Text style={{ fontFamily: "Inter_700Bold", fontSize: 20, color: G.muted }}>
                  {viewStats.lastMonth}
                </Text>
              </View>
            </View>
            <Text style={{ fontFamily: "Inter_400Regular", fontSize: 11, color: G.muted, marginTop: 6 }}>
              * La semaine en cours (barre translucide) est incomplète.
            </Text>
          </SectionCard>
        )}
